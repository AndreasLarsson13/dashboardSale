...
// Leveransalternativ med översättningar
// Definieras HÄR så att den är tillgänglig för både GeneralInfo och för att skickas som prop.
const deliveryTimeOptions = [
  { value: '3-5_days', label: { se: '3-5 dagar', en: '3-5 days', fi: '3-5 päivää' } },
  { value: '5-10_days', label: { se: '5-10 dagar', en: '5-10 days', fi: '5-10 päivää' } },
...
const GeneralInfo = ({ product, setProduct }) => {
  ...
  const [categoryPaths, setCategoryPaths] = useState(() => {
    ...
    // Prioritera product.categoryPaths om det är i det önskade array-av-arrayer formatet
    ...
    // Fallback: Hantera äldre format där product.category (array av kapslade objekt)
    // eller product.categoryPath (enstaka platt array) kan finnas.
    ...
    // Se till att det alltid finns minst en tom sökväg för att visa en väljare
  ...
  const [sellInCountries, setSellInCountries] = useState(() => {
    // Definiera en hjälpfunktion för att normalisera deliveryTime till dess 'value' sträng
    const getNormalizedDeliveryTimeValue = (deliveryTimeData) => {
      if (typeof deliveryTimeData === 'object' && deliveryTimeData !== null) {
        // Om det är ett label-objekt (från backend), försök hitta dess 'value' sträng
        const foundOption = deliveryTimeOptions.find(opt => opt.label.se === deliveryTimeData.se); // Matchar på svensk label
        return foundOption ? foundOption.value : ''; // Returnera 'value' strängen eller tom
      }
      // Om det redan är en sträng (vår nya state-hantering) eller undefined/null
  ...
  useEffect(() => {
    const cleanedCategoryPaths = categoryPaths.map(path => path.filter(Boolean)).filter(path => path.length > 0);

    const nestedCategoriesForProduct = cleanedCategoryPaths
      .map(path => buildNestedCategoryStructure(path, categoriesData))
      .filter(Boolean);

    // Konvertera sellInCountries för att skicka till backend
    ...
    for (const countryCode in sellInCountries) {
      if (sellInCountries.hasOwnProperty(countryCode)) {
        ...
        let deliveryTimeOutput = countryData.deliveryTime; // Detta är strängvärdet (t.ex. '3-5_days')

        // Omvandla till label-objekt vid skickande till backend
        if (typeof countryData.deliveryTime === 'string' && countryData.deliveryTime !== '') {
          ...
          if (foundOption) {
            deliveryTimeOutput = foundOption.label; // Spara hela label-objektet
          } else {
            deliveryTimeOutput = countryData.deliveryTime; // Fallback
          }
        }
        else if (typeof countryData.deliveryTime === 'object' && countryData.deliveryTime !== null) {
          deliveryTimeOutput = countryData.deliveryTime; // Behåll om det redan är ett label-objekt
        }
    ...
    const updatedFields = {
      category: nestedCategoriesForProduct, // Kapslade objekt för product.category
      categoryPaths: cleanedCategoryPaths, // Platta arrayer för product.categoryPaths (sökning/UI)
      currency,
      searchKeywords,
      shippingCurrency,
      specialProductData,
      shippingSpecial: { ...shippingSpecial, enabled: specialShippingEnabled },
      sellInCountries: sellInCountriesForBackend, // Använd den omvandlade datan
    };
    ...
    sellInCountries, // Beroende av sellInCountries för att trigga omvandling för backend
  ...
  // --- Hanterare för leveranstid (sparar 'value' strängen från dropdownen) ---
  const handleSellInCountryDeliveryTimeChange = useCallback((e, country) => {
    const selectedValue = e.target.value; // Detta är nu strängen (t.ex. '3-5_days')
    setSellInCountries((prev) => ({
      ...prev,
      [country]: {
        ...prev[country],
        deliveryTime: selectedValue, // Spara strängen direkt
      },
    }));
  }, []);
  ...
  // --- Hanterare för den dynamiska kategoriväljaren (för flera träd) ---
  const handleCategoryPathsChange = useCallback((pathIndex, level, value) => {
    setCategoryPaths(prevPaths => {
      const newPaths = [...prevPaths];
      const currentPath = [...newPaths[pathIndex]];

      currentPath[level] = value;
      // Återställ alla efterföljande kategorier om en tidigare ändras
  ...
  const addCategoryPath = useCallback(() => {
    setCategoryPaths(prevPaths => [...prevPaths, []]); // Lägg till ett nytt tomt kategoriträd
  }, []);
  ...
  const removeCategoryPath = useCallback((indexToRemove) => {
    setCategoryPaths(prevPaths => {
      ...
      if (filteredPaths.length === 0) {
        return [[]]; // Alltid minst en väljare
      }
  ...
  const isFormCompleted =
    (categoryPaths.length > 0 && categoryPaths[0].length > 0 && categoryPaths[0][0] !== '') && // Minst en kategori är vald
    currency &&
      ...
      {isGeneralInfoOpen && (
        <div>
          ...
          <ShippingAndSalesCountries
            ...
            // currentLanguage={yourActualLanguageState} // Om du har en global språk-state
          />
            ...
            {categoryPaths.map((path, index) => (
              <div key={index} style={{ border: '1px dashed #ccc', padding: '10px', marginBottom: '15px', borderRadius: '5px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                  <h4 style={{ margin: 0 }}>Kategoriväg {index + 1}</h4>
                  {categoryPaths.length > 1 && (
                    <button
                      onClick={() => removeCategoryPath(index)}
                      style={{ background: 'none', border: 'none', color: 'red', cursor: 'pointer', fontSize: '1.2rem' }}
                      title="Ta bort detta kategoriträd"
                    >
                      &times;
                    </button>
                  )}
                ...
                <CategorySelector
                  ...
                  onCategoryChange={(level, value) => handleCategoryPathsChange(index, level, value)}
